...
st.caption(
    ...
    "Gemini Canlı Sentiment + Evrensel Baseline + Kalite Denetimi | V10.1"
)
...
with st.sidebar.expander("⚖️ Skor Ağırlıkları (V10.1)"):
...
# ============================================================
# CANLI GEMINI DUYARLILIK (MARKET SENTIMENT) MOTORU - AI BAYRAKLI
# ============================================================
...
def fetch_market_sentiment(investment_area: str, api_key: str) -> dict:
    ...
    if not api_key or not GENAI_AVAILABLE:
        # API Anahtarı yoksa kural tabanlı simülasyon (ai_active = False)
        area_upper = area.upper()
        if "YABANCI TEKNOLOJİ" in area_upper or "YABANCI" in area_upper:
            return {"score": 38, "label": "Negatif (Kâr Satışı)", "ai_active": False}
        elif "ALTIN" in area_upper or "GÜMÜŞ" in area_upper or "KIYMETLİ" in area_upper:
            return {"score": 82, "label": "Güçlü Pozitif (Faiz İndirimi)", "ai_active": False}
        elif "PARA PİYASASI" in area_upper or "BORÇLANMA" in area_upper:
            return {"score": 65, "label": "Pozitif (Sabit Getiri)", "ai_active": False}
        elif "HİSSE" in area_upper or "BIST" in area_upper:
            return {"score": 54, "label": "Dengeli / Pozitif Beklenti", "ai_active": False}
        return {"score": 50, "label": "Nötr / Kural Tabanlı", "ai_active": False}
...
"""
        ...
        return {
            ...
            "ai_active": True # Gemini başarıyla çalıştı
        }
    except Exception as exc:
        return {"score": 50, "label": f"Nötr (API Hatası: {str(exc)[:20]})", "ai_active": False}
...
def build_http_session() -> requests.Session:
    ...
    session.headers.update({"User-Agent": "KGDM3-Fon-Analiz/10.1", "Accept": "application/json,text/html"})
    return session
...
def compute_fund_metrics(series: pd.DataFrame, fund_code: str, fund_kind: Optional[str] = None, fund_title: Optional[str] = None):
    ...
    prices, dates, aums, invs = df["price"].tolist(), df["date"].dt.strftime("%d.%m").tolist(), df["aum"].tolist(), df["investors"].tolist()
...
def fetch_and_compute_one_fund(code: str, universe: pd.DataFrame, meta_map: dict, valor_dict: dict):
    ...
    metrics["valor"], metrics["source"], metrics["kind"] = valor_dict.get(code, 0.0), source, meta.get("kind", DEFAULT_FUND_KIND)
...
def calculate_security_scores(funds: List[dict], reference: dict):
    ...
    for kind, indices in by_kind.items():
        ...
        for local_i, fund_idx in enumerate(indices):
            ...
            s = 50.0 + SECURITY_SCALE["aum"] * SECURITY_WEIGHTS["aum"] * local_aum_z + SECURITY_SCALE["investor"] * SECURITY_WEIGHTS["investor"] * local_inv_z
            ...
            if f.get("structural_hhi") is not None and f.get("structural_hhi") > 25.0: s -= min((f.get("structural_hhi") - 25.0) * 0.35, MAX_CONCENTRATION_PENALTY)
...
def calculate_market_relative_momentum(funds: List[dict], reference, window: int):
    for f in funds:
        ...
        rets, prc = f["daily_returns"][-window:], f["prices"][-(window + 1):]
        if not rets: f["market_momentum"] = None; continue
        ...
        f["_final_mean_return"], f["_final_sharpe"] = m_r, m_r / vol if vol > 1e-12 else 0.0
        f["_final_cumulative"], f["_final_max_dd"], f["volatility"] = cum, dd, vol
...
def calculate_trend_scores(funds: List[dict], api_key: str = "") -> int:
    ...
    for f in funds:
        f["dates"], f["daily_returns"], f["prices"], f["running_trend_momentum"] = f["dates"][-n_days:], f["daily_returns"][-n_days:], f["prices"][-(n_days + 1):], []
...
def finalize_decisions(funds: List[dict], api_key: str = ""):
    for f in funds:
        ...
        sent_data = fetch_market_sentiment(f.get("investment_area"), api_key)
        sent = sent_data["score"]
        f["sentiment_score"], f["sentiment_label"], f["sentiment_ai_active"] = sent, sent_data["label"], sent_data.get("ai_active", False)
...
def create_excel_output(wb, ws_list, all_funds, common_n_days):
    ...
    daily_headers = []
    for day in last_5_dates: daily_headers.extend([f"{day} Karar Skoru", f"{day} Model Kararı"])
...
st.subheader("📈 KAZRİSK Portföy Özeti (V10.1)")
...
st.subheader("📊 Analiz Sonuçları (V10.1)")
...
st.success(f"✅ V10.1 Analiz tamamlandı. Toplam {len(eligible)} fon işlendi.")
st.download_button(
    label="📥 KAZRİSK V10.1 Excel İndir",
    data=output,
    file_name="fonlar_KGDM3_KAZRISK_FINAL_V10_1.xlsx",
...
if SHOW_DIAGNOSTICS:
    ...
    diagnostic_rows = []
    for item in eligible:
        # Yeni Sütun: AI Bayrağı
        ai_status = "🟢 Aktif (Canlı API)" if item.get("sentiment_ai_active") else "🔴 Pasif (Kural Tabanlı)"
