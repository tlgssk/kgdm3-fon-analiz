...
# ============================================================
# YENİ NESİL GEMINI SDK (google-genai) DUYARLILIK MOTORU
# ============================================================
...
SADECE geçerli bir JSON objesi üret: {{"Alan Adı": {{"score": 75, "label": "Kısa gerekçe"}}}}"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for area in areas:
                if area in parsed_data:
                    result_map[area] = {"score": int(clamp(safe_float(parsed_data[area].get("score", 50)), 0.0, 100.0)), "label": str(parsed_data[area].get("label", "Nötr")), "ai_active": True, "ai_reason": "Requests Başarılı"}
...
# ============================================================
# MULTI-CRAWLER VERİ ÇEKME MOTORU (TEFAS, CLOUDSCRAPER, İŞ YATIRIM)
# ============================================================
...
def fetch_tier3_isyatirim(code: str, start, end):
    ...
    try:
        res = requests.get(url, params=params, headers={"Accept": "application/json"}, timeout=HTTP_TIMEOUT)
        status.status_code = res.status_code
        status.elapsed_ms = int((time.time() - t0) * 1000)
...
calc_funds, failed = [], []
prog = st.progress(0, "API/Cloudscraper ile veriler kazınıyor...")
...
with st.spinner("📊 V14 Modeli (Gemini Toplu Sentiment + Multi-Crawler) Hesaplanıyor..."):
