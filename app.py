...
st.caption(
    ...
    "Kaynak Tanılama + Momentum + Risk + Likidite | V9.0"
)
...
# ============================================================
# V9.0 VERİ KALİTESİ + MATEMATİKSEL TUTARLILIK DENETİMİ
# ============================================================
...
def validate_structural_data(fund: dict) -> Dict[str, Any]:
    issues = []
    dist = fund.get("structural_distribution") or fund.get("portfolio_distribution") or {}
    weights = []
    ...
    total = sum(weights) if weights else None

    if weights and not (STRUCTURAL_SUM_MIN <= total <= STRUCTURAL_SUM_MAX):
        issues.append(f"Dağılım toplamı {total:.2f}%")
...
def create_excel_output(
    ...
    for item in all_funds_for_output:
        ...
        row_data = [
            ...
            round(item["structural_hhi"], 4)
            if item.get("structural_hhi") is not None else None,
            item.get("n_days"),
        ]
...
with st.spinner(
    "📊 Veri kalite denetimi + hibrit skor hesaplanıyor..."
):
    for _fund in eligible_funds:
        audit_fund_data(_fund)
...
for item in all_funds_for_output:
    ...
    display_rows.append({
        ...
        "Yapısal HHI":
            (
                round(item["structural_hhi"], 4)
                if item.get("structural_hhi") is not None
                else None
            ),
    })
...
st.download_button(
    ...
    data=output,
    file_name=(
        "fonlar_KGDM3_KAZRISK_FINAL_V9_0.xlsx"
    ),
    mime=(
...
# V9.0 RELEASE NOTE
# Bu sürüm veri yokluğunu sayı uydurarak kapatmaz. Kaynaklar, endpoint'ler,
# fon evreni ve piyasa koşulları değişebileceğinden mutlak kusursuzluk
# garanti edilemez; bunun yerine kaynak başarısızlığı ve veri tutarsızlığı
